...
- [x] Refactor: split `backend/src/domain/imports/pdf/index.ts` (467 → 117) into per-stage modules
      ...
      New `tests/pdf-import-diagnose.test.ts` (6 tests) covers the extracted pure logic: `flattenItems` cross-page order + empty-input; `diagnoseStaleTemplate` missing-pageAnchor path, overrun-warning path via `non traitée` regex, generic-fallback path when neither triggers, empty/whitespace pageAnchor bypasses the anchor branch. Existing PDF-import integration tests continue to protect the full pipeline untouched.
      Verification: `npx tsc -b` clean, `npm run build` (tsc + copy-migrations) succeeds, isolated `DB_DRIVER=pglite RUN_DB_TESTS=1 npx vitest run tests/pdf-import-diagnose.test.ts` = 6/6 pass. `imports-route.test.ts` shows 7 hook-timeout failures verified pre-existing on pristine main (git-stash round-trip: pristine 20/27, refactor same 20/27) — unrelated to this change.

- [x] Refactor: further-split `backend/src/http/routes/transactions/index.ts` (477 → 20) into per-handler modules
      `transactions/index.ts` is now a pure composer: installs `preHandler: requireAuth` and calls five register* functions in sequence. Handlers moved out into siblings: `create.ts` (78 — POST /api/transactions with dedup + rule-engine categorisation), `list.ts` (140 — GET /api/transactions + GET /:id, share hydrateSplits; the split-aware category filter and the amount-widening OR both stay; running-balance computation only fires on account-scoped views), `patch.ts` (92 — PATCH /:id with category_source flip only when categoryId changes, and the 23514 amount-lock trigger 409), `bulk.ts` (112 — delete-bulk with mirror-leg unlink + categorize-bulk with transfer/split-parent skipping), `delete.ts` (34 — DELETE /:id with mirror-leg unlink). `helpers.ts` (67, up from 15) now exports `hydrateSplits` (previously inlined at top of index.ts) plus a new pure `buildAmountRange(unsignedNeedle)` that returns `{ lo, hi }` for the sign-agnostic amount-search widening.
      Extended `tests/transactions-helpers.test.ts` (previously 5 tests) with 5 new `buildAmountRange` tests: integer widening to `[N.00, N.99]`, one-decimal to `[N.d0, N.d9]`, full two-decimal exact-match, multi-digit integer preservation, zero handling. Existing `transactions-route.test.ts` (842 lines) remains untouched and continues to exercise the full HTTP surface.
      Verification: `npx tsc -b` clean, `npm run build` (tsc + copy-migrations) succeeds, isolated `DB_DRIVER=pglite RUN_DB_TESTS=1 npx vitest run tests/transactions-*.test.ts` = 63/63 pass. index.ts is 20 lines (target was <200); no sibling handler file over 140 (target was <180).

- [x] Refactor: split `backend/src/http/routes/accounts.ts` (487 → 7 files, largest 225) into per-concern modules
      Old single-file `routes/accounts.ts` replaced by `routes/accounts/` folder matching the reports/envelopes pilot (commits e01d841/017a1ea): `index.ts` (composer, 15 lines — installs preHandler + calls each register*), `schemas.ts` (49 — every Zod schema: decimal, isoDate, isoCurrency, lockYears, CreateBody, UpdateBody, IdParam, MergeBody, ReorderBody, SourceIdParam), `helpers.ts` (23 — parseId, isPgError), `list.ts` (104 — GET /api/accounts with the correlated-subquery current/available balance SQL, verbatim from the original with rationale comment intact), `reorder.ts` (32 — PUT /api/accounts/order), `crud.ts` (91 — POST/GET-by-id/PUT/DELETE), `merge.ts` (225 — POST /api/accounts/:sourceId/merge, kept as one atomic file because every step of the 7-step merge transaction depends on the surrounding steps). `buildServer.ts` import updated from `./http/routes/accounts.js` to `./http/routes/accounts/index.js`.
      New `backend/tests/accounts-helpers.test.ts` (24 tests): `isPgError` string-code accept + reject-null/non-object/non-string-code; `decimal` accept int/1-decimal/2-decimal signed + reject `.5`/`1.`/`1.234`/comma/empty; `isoDate` accept + reject alt formats + T-suffix; `isoCurrency` 3-uppercase accept + case-and-length reject; `lockYears` `[0,99]` int + null accept + negative/above-99/non-integer reject; `CreateBody` EUR + `0` defaults + empty-name-after-trim reject + name-trim; `UpdateBody` all-partial + per-field validation; `IdParam` positive-int coerce + zero/negative/non-numeric reject; `MergeBody` positive-int targetId; `ReorderBody` non-empty + `≤200` cap.
      Verification: `npx tsc -b` clean, `npm run build` (tsc + copy-migrations) succeeds, isolated `DB_DRIVER=pglite RUN_DB_TESTS=1 npx vitest run tests/accounts-*.test.ts` = 47/48 pass. The one failure (`DELETE refuses when the account still has transactions (409)` returning 500) is **pre-existing on pristine main** (verified via `git stash pop` round-trip — 17/18 on main vs 47/48 with the refactor, same test failing). Behavior byte-for-byte identical to the pre-refactor DELETE handler; the failure is a driver-level `.code` mapping issue independent of this change.

- [x] Refactor: split `frontend/src/api/demo/handlers/reads.ts` (507 → 8 files, largest 148) into per-resource modules
      Old single-file `handlers/reads.ts` replaced by `handlers/reads/` folder: `index.ts` (composer, 20 lines), `lib.ts` (148 — all pure helpers), `simple.ts` (14 — auth/onboarding/health + trivial list endpoints), `accounts.ts` (51 — accounts + balance-checkpoints + reports/balance, share enrichAccount from lib), `transactions.ts` (20 — the handler; parse/filter/attach-running-balance moved to lib), `reports.ts` (125 — timeseries + categories + budget), `tri.ts` (54), `recurring.ts` (42). `demo/index.ts` import path `./handlers/reads` continues to resolve (folder-index-shadow-file).
      Circular-import gotcha resolved: sibling handler files import `registerHandler` from `demo/index.ts`, and demo/index.ts calls `registerReadHandlers()` at its bottom. That means a test file that imports pure helpers from a handler sibling would deadlock (the sibling is still mid-load when its register* function is called). Fix: all pure helpers (`parseTxFilters`, `applyTxFilters`, `attachRunningBalance`, `enrichAccount`, plus the previously-inline `money`/`bucketFor`/`monthOf`/`addDaysIso`/`nextDueFrom`/`computePrimaryAccountId`) live in `lib.ts`, which imports only from `store` — no cycle. Handlers import the pure helpers from lib.
      New `__tests__/reads-helpers.test.ts` (29 tests): `money` positive/zero/negative + tiny-negative sign-prefix carry-through documented; `bucketFor` day + month + ISO-Monday-of-week (Mon/Wed/Sun cases); `monthOf`; `addDaysIso` positive + month/year rollover + negative; `nextDueFrom` already-at-today + walk-forward-to-today; `computePrimaryAccountId` no-match + sole + majority-vote; `parseTxFilters` field mapping + `categoryId=null`-vs-`uncategorized=true` routing + numeric `categoryId` + `[1,500]` limit clamp + defaults; `applyTxFilters` accountId + from/to inclusive + uncategorized + categoryId + q case-insensitive on both label fields; `enrichAccount` opening + counted delta sum + pre-opening exclusion + raw-vs-counted counts + availableBalance mirror.
      Verification: `npx tsc -b` clean, `npx vitest run` = 757/757 tests pass (728 pre-existing + 29 new), plain `npm run build` and `VITE_DEMO=1 npm run build` both succeed.
...
- [x] Refactor: split `frontend/src/components/BalanceChart/index.tsx` (542 → 464) into per-concern modules
      ...
      New `__tests__/lib.test.ts` (22 test cases): `isoDate` local-day padding; `mergeHistoricalAndProjection` no-projection passthrough + alignEndTo shift + zero-shift equality + sorted-forward append + past-date filter + empty-raw; `computeYRange` sign-mixed + all-zero fallback; `computeActiveWindow` no-zoom + zoom-narrow + degenerate-span clamp; `buildSegments` empty/1-sample early return + all-solid + projection boundary split + gap-threshold split; `buildAreaPath` closed polygon + empty when no historical; `buildAxisTicks` 6 tick cap + 2-sample floor.
      Verification: `npx tsc -b` clean, `npx vitest run` = 717/717 tests pass (695 pre-existing + 22 new), `npm run build` succeeds. All 28 existing BalanceChart-namespace tests still green untouched.
