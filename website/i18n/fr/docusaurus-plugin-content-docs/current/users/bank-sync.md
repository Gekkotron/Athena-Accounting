...
3. Créez une **application**. Pour l'URL de redirection demandée, utilisez l'adresse de votre Athena suivie de `/bank-sync/callback` — la valeur exacte est affichée dans *Données → Synchronisation bancaire*. Téléchargez la clé privée RS256 générée par le Control Panel pour cette application.
4. Dans Athena, ouvrez *Données → Synchronisation bancaire* et collez l'**identifiant d'application** et la **clé privée** (le fichier PEM complet, lignes `BEGIN`/`END` incluses). Athena valide la paire auprès d'Enable Banking avant de l'enregistrer ; la clé est chiffrée au repos et n'est plus jamais affichée, renvoyée ni journalisée.
...
- **« URL uses unsupported scheme » au moment de whitelister l'URL de redirection** — le Control Panel d'Enable Banking refuse les adresses en `http://`. Whitelistez la jumelle `https://` de votre adresse Athena (p. ex. `https://192.168.1.20:8080/bank-sync/callback`) — le panel ne la contacte jamais, elle n'a donc pas besoin d'exister. Après validation du consentement, le navigateur échoue à charger cette page `https://` ; le code à usage unique reste dans la barre d'adresse. Remplacez-y `https` par `http` et validez (vous arrivez sur la page d'association d'Athena), ou collez l'URL complète dans *« Finaliser manuellement »*. Autre piste : `http://localhost:<port>/bank-sync/callback` peut être accepté — utilisable directement sur le serveur ou via un tunnel SSH.
- **La redirection n'atteint jamais Athena** (l'URL whitelistée ne correspond pas à l'adresse à laquelle vous consultez Athena, ou le Control Panel a refusé votre URL LAN) — le code d'autorisation figure quand même dans la barre d'adresse de la page finale. Copiez cette URL complète et collez-la dans *Données → Synchronisation bancaire → « Finaliser manuellement »* ; Athena en extrait le code et crée la connexion. Rappel : l'URL de redirection ne doit être accessible que par **votre navigateur**, jamais par Enable Banking — une adresse LAN comme `http://192.168.1.20:8080/bank-sync/callback` est le cas normal.
- **La page de retour affiche une erreur** — le code d'autorisation est à usage unique et de courte durée ; relancez la connexion depuis Données → Synchronisation bancaire.
