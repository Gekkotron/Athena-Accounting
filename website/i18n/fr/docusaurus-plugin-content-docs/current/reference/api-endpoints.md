...
| `GET`  | `/api/backup/destination` | Statut de la destination de sauvegarde distante : type (`webdav`/`folder`), configuration non secrète, heure planifiée, dernière/prochaine exécution, dernière erreur. Les secrets (mot de passe WebDAV, phrase secrète) ne sont jamais renvoyés. |
