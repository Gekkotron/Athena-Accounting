...
| `GET`  | `/api/backup/destination` | Statut de la destination de sauvegarde distante : type (`webdav`/`folder`/`ftp`), configuration non secrète, heure planifiée, dernière/prochaine exécution, dernière erreur. Les secrets (mot de passe WebDAV, phrase secrète) ne sont jamais renvoyés. |
| `PUT`  | `/api/backup/destination` | Configure la destination. Valide par une vraie écriture de test + suppression sur la cible avant de persister ; 502 avec un `detail` lisible en cas d'échec. Les secrets sont chiffrés au repos. |
| `DELETE` | `/api/backup/destination` | Supprime la destination et ses secrets chiffrés. Les fichiers déjà poussés restent sur la destination. |
| `POST` | `/api/backup/destination/run-now` | Exécute une sauvegarde immédiatement avec la configuration stockée : construit le dump, le scelle (`enc1`), l'envoie, élague au-delà de `keepLast`. Renvoie le nom de fichier envoyé ; 409 si non configuré. |
