...
| `BANK_SYNC_AUTO` | `1` | `1`/`0`, `true`/`false` | Nightly unattended bank sync for users who configured Enable Banking credentials in *Réglages → Synchronisation bancaire*. Set to `0` to disable the scheduler and only sync on demand from the UI. See [Bank sync](../users/bank-sync.md). |
