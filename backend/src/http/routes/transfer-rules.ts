...
export async function transferRulesRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get('/api/transfer-rules', async () => {
    const rows = await db.select().from(transferRules).orderBy(transferRules.id);
  ...
  app.post('/api/transfer-rules', async (req, reply) => {
    ...
    try {
      const [created] = await db.insert(transferRules).values(parsed.data).returning();
